...
class StressTest:
    ...
    def __init__(
            ...
            url: str,
            total_requests: int,
            callback: tp.Callable[[int, int], None]
    ):
        self._completed_requests: int = 0
        self._load_test_future: tp.Optional[concfu.Future] = None
        self._loop = loop
        self._url = url
    ...
    def start(self):
        self._load_test_future = \
            asyncio.run_coroutine_threadsafe(self._make_requests(), self._loop)

    def cancel(self):
        if self._load_test_future:
            self._loop.call_soon_threadsafe(self._load_test_future.cancel)

    async def _get_url(self, session: aiohttp.ClientSession, url: str):
        try:
